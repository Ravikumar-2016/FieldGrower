...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['delete_crop'])) {
    ...
    // Validate the input
    if (!empty($cropname)) {
        // Prepare SQL to delete crop based on username and cropname
        $delete_sql = "DELETE FROM allusercropdetails WHERE username = ? AND cropname = ?";

        // Use prepared statements to prevent SQL injection
        $stmt = $conn->prepare($delete_sql);
        $stmt->bind_param("ss", $username, $cropname);
        ...
        try {
            ...
            if ($stmt->execute() && $stmt->affected_rows > 0) {
                ...
                $error_message = "Crop not found!";
            }
        } catch (mysqli_sql_exception $e) {
            $error_message = "An error occurred: " . $e->getMessage();
        }
    } else {
        $error_message = "Crop name is required to delete a crop.";
    }
}
?>
...
<div class="center-container">
    <form class="delete-crop-form" method="POST" action="">
        <div class="form-group">
            <label for="cropname">Crop Name</label>
            <input type="text" id="cropname" name="cropname" placeholder="Enter crop name to delete" required>
        </div>
        <div class="submitbtn">
            <button type="submit" class="search-button" name="delete_crop">Delete Crop</button>
        </div>
    </form>
...
<script>
    // Automatically hide the success message after 1100ms
    setTimeout(() => {
